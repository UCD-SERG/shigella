...
# mean is serodynamics:::ab(); a simulated value is exp(rnorm(1, log(mu_hat), sd)). # nolint: line_length_linter.
#' @keywords internal
#' @noRd
.ppc_simulate <- function(data, raw_dat, n_sim) {
  ...
  # TODO(serodynamics export): see REPRODUCIBILITY.md "Known tech debt: serodynamics internal calls".
  obs_dat <- raw_dat |>
