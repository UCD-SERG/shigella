...
.fig4_isotype_panel <- function(iso, sid, model_overall, model_sero,
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_ribbon(
      ...
      fill = .model_colours[["Overall"]], alpha = 0.12
    ...
    ggplot2::geom_ribbon(
      data = pred_sero,
      ggplot2::aes(x = .data$t, ymin = .data$lo, ymax = .data$hi),
      fill = .model_colours[["Serotype-specific"]], alpha = 0.12
    ) + # nolint: object_usage_linter, line_length_linter.
    ggplot2::geom_line(
      data = pred_sero,
      ggplot2::aes(
    ...
    if (nrow(pred_cmb) > 0) {
      p <- p +
        ggplot2::geom_ribbon(
          data = pred_cmb,
          ggplot2::aes(x = .data$t, ymin = .data$lo, ymax = .data$hi),
          fill = .model_colours[["Combined flexneri"]], alpha = 0.10
        ) + # nolint: object_usage_linter, line_length_linter.
        ggplot2::geom_line(
          data = pred_cmb,
          ggplot2::aes(
  ...
  p +
    ggplot2::scale_color_manual(
      name = "Model", values = .model_colours, # nolint: object_usage_linter, line_length_linter.
      breaks = names(.model_colours)
    ) + # nolint: object_usage_linter, line_length_linter.
    ggplot2::scale_linetype_manual(
      name = "Model", values = .model_linetypes, # nolint: object_usage_linter, line_length_linter.
      breaks = names(.model_linetypes)
    ) + # nolint: object_usage_linter, line_length_linter.
    ggplot2::scale_y_log10(labels = scales::label_comma()) +
    ggplot2::coord_cartesian(xlim = xlim) +
    ggplot2::labs(
      x = "Days since symptom onset", y = "MFI (log scale)",
...
build_figure4_row <- function(sid, model_overall, model_sero,
  ...
  panel <- function(iso) {
    .fig4_isotype_panel(
      iso, sid, model_overall, model_sero,
      data_overall, data_sero, model_combined,
      antigen_label, xlim, times
    )
  }
