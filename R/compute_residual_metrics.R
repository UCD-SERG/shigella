...
#'
#' Computes residuals between observed antibody measurements and posterior median
#' predictions evaluated at the observed time points. Returns pointwise residuals
#' or aggregated error metrics (MAE, RMSE, SSE) at multiple summary levels.
#'
...
#'   \code{serodynamics::as_case_data()} (must contain \code{id} and \code{antigen_iso}
#'   columns, and time/value attributes).
#' @param ids Character vector of subject IDs to include (matched against \code{dataset$id}
#'   and \code{model$Subject}).
...
#' @param scale Scale on which to compute residuals. One of \code{"original"} or \code{"log"}.
#'   If \code{"log"}, residuals are computed on the natural log scale and observations/predictions
#'   \eqn{\le 0} are removed.
#' @param summary_level Level at which to summarize metrics. One of:
#'   \describe{
#'     \item{\code{"pointwise"}}{Return pointwise residuals at each observed time point.}
#'     \item{\code{"id_antigen"}}{Summarize by \code{id} and \code{antigen_iso}.}
#'     \item{\code{"antigen"}}{Summarize by \code{antigen_iso} only.}
#'     \item{\code{"overall"}}{Single summary across all included observations.}
#'   }
#'
#' @return A tibble. For \code{summary_level = "pointwise"}, returns per-observation residuals.
#' Otherwise returns MAE, RMSE, SSE, and \code{n_obs} at the requested summary level.
#'
#' @details
#' The posterior predictive summary uses the posterior median at each observed time point,
#' with a 95\% credible interval computed from draw-level predictions. Predictions are
#' generated by \code{\link{predict_posterior_at_times}}.
#'
#' @examples
#' \dontrun{
...
compute_residual_metrics <- function(model,
  ...
  if (summary_level == "pointwise") {
    return(residual_data |>
      dplyr::select("id", "antigen_iso", "t", "obs", "pred_med",
