...
This is not because the established implementation generates from the product model — it does not, and its documentation is explicit that each individual's biomarkers share one latent infection history — but because its generative model differs from the estimator in other ways, set out below.
