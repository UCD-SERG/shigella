...
Seroincidence estimates the rate of new infections in a population from a single blood draw per participant, and where blood culture surveillance is out of reach it is the practical alternative.
...
We derived a joint likelihood in which a participant's biomarkers share one infection time and their kinetic parameters are drawn as a pair, validated it against the implementation in current use, and evaluated the calibration of that implementation in a coverage study across an eightfold range of transmission intensity.
The joint form relaxes two assumptions at once, and we separated them so that their contributions could be measured rather than argued.
The forms were applied to SEES cross-sectional serosurveys covering 2,613 participants in Bangladesh, Nepal, and Pakistan.

A nominal 95% interval built by adding log-likelihoods contained the true incidence 89.7% of the time; the joint form restored calibration at 98.0%, at a cost in width of about a quarter that was constant across transmission intensity.
Accuracy was almost unchanged, at relative root mean squared errors of 10.16% and 9.53%.
...
Intervals obtained by summing isotype log-likelihoods are too narrow, by an amount that roughly doubles the nominal error rate.
...
The kinetic model is the two-phase curve estimated in the companion analysis [@lee2026correlated].
...
$$
\underbrace{\text{one infection time per participant}}_{\text{assumption ①, independent of the kinetic model}} \qquad \underbrace{\text{paired kinetic parameters}}_{\text{assumption ②, supplied by the kinetic model}}
$$ {#eq-two-assumptions}
...
Deciding where to introduce them requires knowing where transmission is highest, and that is precisely the information that is missing in most high-burden settings: blood culture surveillance requires laboratory infrastructure and sustained financing, and even where it is in place it detects only the fraction of infections that produce symptoms, reach care, and yield a positive culture.
Seroincidence estimation was developed to fill that gap: one blood draw per participant, analyzed against a model of how antibodies behave after infection, yields an infection rate for the population — including the asymptomatic and unrecognized infections that never reach a clinic [@simonsen2009; @teunis2020; @aiemjoy2022; @aiemjoy2023serodynamics].
...
Its own description of the assumption is explicit:
...
The last clause is the one this chapter examines.
...
Its simulator gives each person one infection time and one set of kinetic parameters, and reads both isotypes from that single state.
...
For a method whose output feeds burden estimates and vaccine introduction decisions, that is a specific and consequential kind of error.
...
Assumptions made for tractability in seroepidemiology have not historically been harmless, and it is worth checking this one before it is carried into further applications.

## Why width alone is not the question {#sec-ch3-coverage}

An interval can be widened by anyone.
Multiplying it by 1.3 makes it wider without making it right.
...
A statement that one interval is 40% wider than another is a statement about that code.
A statement that a nominal 95% interval covers 64% of the time is a statement about every published estimate computed the same way.
...
It is reported whether or not it does, and if it does not, the disagreement between it and the coverage study is itself a finding that has to be explained rather than omitted.

The effect could be small.
Two considerations suggested it might be.
The inversion uses the decay phase only, so the pre-infection level and the time to peak — among the most strongly correlated parameters in the companion analysis — do not enter.
...
and the kinetic parameters are paired, with the two assumptions separated so that their contributions can be attributed individually (@eq-two-assumptions).
2. An implementation and its validation. No joint likelihood exists in the available software, so
it was written; it is validated against the existing implementation in the single-biomarker case, where the two forms are the same integral and must agree exactly.
3. A coverage study on simulated cross-sectional data with known $\lambda$, across a range of
incidence rates chosen to span the observed settings, reporting coverage, interval width, and bias for both likelihoods.
4. An application to the SEES cross-sectional serosurveys in three countries. The likelihood
form and the parameter bank are crossed as two separate factors, so that the correlated and independent banks of @sec-ch3-banks enter without any additional model fitting, and the question is whether the epidemiological conclusions — the ranking of countries and the contrast between high- and low-endemicity arms — survive.
...
HlyE IgG and IgA are measured by the same ELISA as in the companion analysis, so the parameter bank and the specimens it is applied to are on one scale.
...
household containing a known case rather than that of the community, and including them would attach a case's household to a population rate.
...
What remains is 2,613 participants, each with exactly one IgG and one IgA measurement and no missing age or concentration.
The household identifier is one-to-one with the participant identifier in these two arms, so no household clustering survives the second rule; 48% are male, ranging from 35% to 53% across the six cells.
The six country-by-endemicity cells are described in @tbl-cells.
Measurements below the detection limit are retained at the limit and enter as censored contributions rather than being dropped, for the reason given in the companion analysis [@lee2026correlated].

## Seroincidence from a cross-sectional sample {#sec-ch3-setup}

The estimator is the one implemented in `serocalculator` [@serocalculator2025], and the notation below follows its methodology vignette so that each step can be checked against it.
For a participant of age $a$ whose infections arrive as a Poisson process of rate $\lambda$ — the seroconversion rate, in infections per person-year, which is the quantity being estimated — the time $\tau$ since the most recent infection has a mixed distribution: an atom at "never infected" and a truncated exponential density otherwise,
...
where $\boldsymbol\theta^{(k)}=(y_0,y_1,t_1,\alpha,r)^{(k)}$, $k=1,\dots,M$, are posterior draws of the kinetic parameters of @eq-curve — the parameter bank.
The inner average is a Monte Carlo approximation to $\int p(y\mid\tau,\boldsymbol\theta)\,p(\boldsymbol\theta)\,d\boldsymbol\theta$: an individual's own kinetics are unknown, so they are integrated over the population distribution, and the bank is the sample that integral is taken over.
...
Write $J$ for the number of biomarkers combined for one participant — $J=2$ throughout this chapter, HlyE IgG and HlyE IgA — and reserve the subscript $j$ for the biomarker and the superscript $(k)$ for the draw from the parameter bank, as the vignette does.
...
Approximating the parameter integral with the bank, using the same draw index for both biomarkers, and exchanging the sum with the integral — which is exact, by linearity — gives the joint likelihood: a single $\tau$ and a single bank index shared across biomarkers,
...
Between @eq-prodform and @eq-joint sits a third arrangement, and it is the one the vignette specifies: the infection time is shared but each biomarker is still averaged over the bank separately.
...
$$
\ell_{\mathrm A}=\ell_{\mathrm{prod}},\qquad \ell_{\mathrm B}=\sum_i\log L_i^{\mathrm{shared}},\qquad \ell_{\mathrm C}=\ell_{\mathrm{joint}} .
$$ {#eq-abc}

These three exhaust the arrangements: with a sum, an integral, and a product to order, form A places the product outermost, form B the integral, and form C the sum.
That ordering is the whole content of the chapter, and it is tabulated in @sec-appB-deriv.
Form B is what makes the two assumptions separable rather than a single change, and @sec-ch3-design uses it for exactly that.
...
The companion analysis's correlation enters here, and only here — not as a parameter of the likelihood but as a covariance taken across rows of the bank.
Two banks with the same marginal distributions can differ entirely in it, which is the relation between the two banks defined next.

Three distinct correlations are now in play and it is worth separating them, because they are easy to conflate and only one of them is a parameter.
The cross-isotype correlation $ ho_c$ is a correlation between kinetic parameters — between the peak a participant would reach in IgG and the peak they would reach in IgA — and it is estimated in the companion analysis of antibody kinetics [@lee2026correlated]. $\widehat{\operatorname{Cov}}_k$ in @eq-covdraws is a covariance between two **density values** at a fixed $\tau$, taken across rows of the bank; it asks whether the rows that explain a participant's IgG well are the same rows that explain their IgA well, and it is computed, not estimated.
The correlation between the two observed log concentrations is a third thing again, and it is neither of the first two: it is the quantity @sec-ch3-res-shift compares against what the bank predicts. **A bank can carry a large $\rho_c$ and still fail to reproduce the observed correlation**, and whether it does is a question about the bank rather than about $\rho_c$.
...
cross-isotype correlation.
This is where the companion analysis enters, and only here.

Assumption ① does not depend on the companion analysis, and this matters for how the chapter's conclusion should be read.
A reader who is unconvinced by the companion analysis still has to accept that a participant has one infection time.
The design of @sec-ch3-design is built so that the contribution of ① can be read off separately, and if it is the larger of the two, the main result of this chapter is independent of the previous one.

## Parameter banks {#sec-ch3-banks}

The banks are posterior draws from the companion analysis, converted to the form the estimator expects with the draw index preserved — that index is the only thing linking a participant's IgG parameters to their IgA parameters, and the pairing in @eq-joint depends on it.
...
The independent bank comes from arm S, the same model specification with $\mathbf c$ held at zero, so the two banks differ in the cross-block alone.
...
The correlation carried by each bank is reported as a check before any of them is used, since a correlated bank that does not carry the correlation, or a shuffled bank that still does, would make every downstream contrast meaningless.

## Study design: two axes, not one {#sec-ch3-design}

The most common misreading of this design is to equate the likelihood form with the chapter.
It is worth being explicit that these are two crossed factors, and that the product likelihood can be run with the correlated bank just as the joint likelihood can be run with the independent one.
...
All three are evaluated by the same function.
...
One contrast was recorded before the results were seen.
The product likelihood cannot see the pairing.
It integrates each biomarker separately, so replacing a correlated bank with one that has identical marginals and destroyed pairing must give the same number — not a close one.
...
With a single biomarker, @eq-joint and @eq-prodform are the same integral.
Setting $J=1$ in both returns @eq-single, so the two implementations must agree:
...
$$
\ell_{\mathrm A}(\lambda)\big\rvert_{J=1}= \ell_{\mathrm B}(\lambda)\big\rvert_{J=1}= \ell_{\mathrm C}(\lambda)\big\rvert_{J=1}.
$$ {#eq-threeform}
...
The gate is described in some detail because its value is easy to underestimate.
...
$\Pr(Y\le y_{\text{low}})$, not a density.
The IgG error was roughly thirty times the IgA error, IgG being the more heavily censored isotype.
...
A second check compares the joint form against the product form under a bank in which the isotypes are independent and the noise is made small: as the noise shrinks both concentrate on the same $\tau$, and the two should converge.

## Numerical evaluation {#sec-ch3-numeric}

Neither likelihood has a closed form in $\lambda$, so both are evaluated on a grid — a fixed ladder of candidate incidence values, spaced logarithmically, over which the log-likelihood is computed point by point.
Everything below concerns making that evaluation fast enough and fine enough.

The integral in @eq-joint is evaluated on a fixed quadrature grid in $\tau$, with participants binned by age so that the weight vector $f(\tau\mid a,\lambda)$ can be reused within a bin.
...
Grid resolution in $\tau$, the age bin width, and the number of bank draws are all reported, and the profile is checked for insensitivity to each.
...
The grid spacing is reported alongside every interval, because it sets the finest difference the method can express: a change smaller than one grid step is not a change the analysis can resolve, and is reported as such, not as a small effect.
...
The design follows the standard for simulation studies [@morris2019]: a Monte Carlo standard error is reported for every performance measure, and what counts as acceptable was fixed before any grid was run.

The simulated sample size is $n=300$, at the lower end of the range the observed cells span — 242 to 596, with a median near 450 (@tbl-cells).
...
**Simulation design.** For each replicate: draw ages by resampling from the observed serosurvey; draw infection status and $\tau$ from @eq-brt at a known $\lambda$; draw one bank index per participant, which is what preserves the pairing; evaluate @eq-curve for both isotypes at that $\tau$; apply the two-stage noise of @sec-appB-noise; and censor at the detection limit.
Both likelihoods are then fitted to the same simulated dataset and both intervals are scored.

The simulator is written out rather than taken from the package.
...
The simulator is validated before use, and against form C rather than against the package.
Once the comparator is internal, the coherent question is no longer whether the simulator reproduces a second codebase — it is whether the simulator and form C are the same model stated in two directions.
The gate therefore asks form C to recover a $\lambda$ drawn from its own generating process, at three values spanning the range, before any coverage replicate is run.
...
They are recorded because they determined the tolerances now in use.
...
**Conditions.** Coverage is evaluated at six values of $\lambda$ spanning the range of estimates obtained in this analysis, with 50 replicates at each and both forms fitted to the same simulated datasets.
...
The difference in coverage between the two likelihoods is tested paired, since both are fitted to the same simulated datasets.
...
The decomposition is reported directly.
For each condition, the coverage that a correctly scaled interval would achieve if it were mis-centered by the observed bias alone is computed and compared against the observed coverage.
...
This is stated here because it constrains what the chapter may claim.
...
The decomposition is run before the discussion is written.

## Application {#sec-ch3-app}

The three forms of @tbl-design and the banks of @tbl-banks are applied to the SEES cross-sectional serosurveys.
Four of the nine combinations are reported, which is what the two contrasts require: the independent bank under form A, which is current practice from end to end, and the correlated bank under each of the three forms.
...
No external benchmark is used, and the omission is deliberate.
...
The parameter bank here comes from prospectively enrolled participants only, with single-visit participants excluded [@lee2026correlated]; kinetics are fitted with one pooled set rather than by age stratum; and the cross-sectional restriction, the censoring convention, and the noise handling are all specified independently.
Two analyses that differ in the estimation sample, the parameter bank, and the preprocessing cannot validate one another, and an apparent agreement between them would be as likely to reflect offsetting differences as correctness.
...
The cluster-robust standard error with the participant as the cluster is computed alongside, as the pre-specified conventional check described in @sec-ch3-honest.
A ratio above one supports the argument; a ratio at or below one does not, and is reported either way.

## Sensitivity analyses {#sec-ch3-sens}

Two features of these serosurveys could produce a spurious difference between countries.
Both are examined with predictions recorded in advance.
...
The informative cell is the country with the least censoring at its native limit, not the one with the most.
...
Earlier work on these cohorts fitted kinetics in three age strata and restricted the serosurveys to participants under 26 [@aiemjoy2022]; a single pooled bank is used here.
That reference is to a methodological choice, not to a set of results — the point is that an alternative handling of age exists and has to be addressed, not that any published number is being reproduced or contested.
The age composition of each cell is therefore tabulated first, without fitting anything, and the restriction is run only where the composition makes it capable of mattering.
A cell that loses under 5% of its participants under the restriction cannot move materially, and reporting that computation-free fact is a result in itself.

The pooled bank is retained rather than refitted by age stratum, for two reasons.
The bank enters both likelihood forms identically, so any error in it is common to both and cannot generate the difference between them — which is the quantity this chapter reports.
And refitting by stratum would divide the companion analysis fit into three, at sample sizes below where that model converges reliably, while that same fit is the source of the simulation truth.
Pooled versus age-stratified kinetics is reported in the manuscript rather than the dissertation, on the advice of the committee.

**Ethical Approval.** This study was approved by the institutional review boards of the Centers for Disease Control and Prevention, Stanford University (39557), and MassGeneral Brigham (2014P002602, 2019P000152, 2013P001965, 2016P000949) in the USA; the Bangladesh Institute of Child Health Ethical Review Committee (01-02-2019); the Nepal Health Research Council Ethical Review Board (391/2018); and the AKU Ethical Review Committee (2019-0410-4188) and the Pakistan National Bioethics Committee (4-87/NBC-341-Amend-revised/19/81) in Pakistan.
...
And one cell has an age structure unlike the rest: the low-endemicity Pakistan arm is 22.0% aged 26 or over against 0.8% or less elsewhere, and it sits on the low side of the Pakistan endemicity contrast, which is why that cell alone is re-analyzed under an age restriction.

A third feature is worth noting because it recurs in @sec-ch3-discussion.
Censoring is far from symmetric between isotypes within a cell: low-endemicity Pakistan has 59.7% of IgG at the limit against 1.0% of IgA, while the Nepal cells have both isotypes heavily censored together.
...
With one biomarker the three forms agree to machine precision.
Evaluated on the high-endemicity Bangladesh sample at five values of $\lambda$, separately for each isotype, the largest disagreement between forms A, B, and C was $5.7\times10^{-14}$ on a log-likelihood of order $10^{2}$, a relative difference of $1.2\times10^{-16}$ (@eq-threeform).
...
Across all six cells the contrast of @eq-nullcheck was $0.0\%$ to the precision of the stored estimate, in six of six.
...
The design of @tbl-banks assumes that the correlated and independent banks have the same marginal distributions, so that any difference between them is attributable to the pairing. @tbl-two-banks is that assumption checked.
...
Against the spread inside either bank — the IgG baseline runs from 10 to 47 — the two are the same set of numbers.

What separates them is not in any column.
It is whether a row is one person: in the correlated bank a row pairs an IgG parameter vector with the IgA vector drawn alongside it, and in the independent bank it does not.
The product form averages each column separately and cannot see the difference; the joint form uses a whole row at a time and can. @tbl-forms is that statement measured.
...
The pooled difference is +8.3 percentage points, and the per-$\lambda$ paired differences are positive at all six values, reaching 2.6 standard errors at $\lambda=0.15$.
...
At 50 replicates that departure is 1.0 standard error and cannot be called either way.
...
Across an eightfold range of transmission intensity the price of the correction varies by two percent of itself.
...
Relative to the product form on the same bank, the joint interval was 30% to 60% wider, with a mean of 46%.

Giving the product form the correlated bank moves the estimate by 2.3% — three cells by exactly zero and three by one step of the $\lambda$ grid.
That is @tbl-two-banks appearing in a second place: the product form reads only the column medians, and those barely differ between the banks.
...
That is the whole reason the shared-time form is computed: $\mathrm B$ gives assumption ① alone, so $\mathrm B-\mathrm A$ is what the shared infection time is worth and $\mathrm C-\mathrm B$ is what the companion analysis's pairing adds on top of it.
...
Sharing one infection time widens the interval in six of six cells and accounts for 76% of the total.
Pairing the draws contributes the remaining quarter, is positive in four of six, and is negative in one.
The larger of the two axes therefore requires nothing from the companion analysis: @eq-two-assumptions separated the assumptions precisely so this could be read off, and the reading is that the principal effect follows from a participant having one infection time.
...
An earlier version of this contrast, computed with the established implementation in the role of form A, gave 79% for the sharing axis. **The internal comparison gives 76%.** Two routes that share no code and no data agree to three percentage points.
...
Pakistan separated under the product and joint forms (1.70 and 1.63) though not under form B.
...
The joint estimate exceeded the product estimate in six of six cells, by 14.2% to 42.4%, with a mean of 28.8%.
...
A 29% separation on observed data therefore reflects a feature of the observed data that the simulation does not contain, and it is reported here as an observation, not as an estimate of anything.

One such feature is measurable.
Simulating from the companion analysis fit at each cell's own fitted incidence, with the same sample size, ages, and noise, predicts a correlation of 0.78 to 0.81 between the two log concentrations.
The observed correlations, the last column of @tbl-cells, were 0.17 to 0.63, lower in six of six cells, with a mean gap of 0.36. @fig-ch3-disc puts the two side by side: in every cell the observed points sit below and to the left of the cloud the bank predicts, which is the discordance seen rather than counted.
...
The exclusion of censored pairs is a choice and the finding does not depend on it: retaining them gives observed correlations of 0.39 to 0.60 against 0.79 to 0.82, still lower in six of six. @sec-ch3-discussion sets out what is known about the origin of the discrepancy and records it as unresolved.
...
Setting a common lower limit at 2, 2.5, and 3 across all countries and both isotypes moved every cell by at most 2.0 steps on a $\lambda$ grid resolved to 4.5%, and by one step in five of six.
The informative cell is high-endemicity Bangladesh, whose IgG censoring rises from 0.2% at its native limit to essentially complete at a limit of 3, and which moved from 0.332 to 0.317.
...
The one cell whose age composition stands out does not drive its contrast.
...
The cluster-robust check did not support the argument, and is reported as such.
With the participant as the cluster, the ratio of the robust to the naive standard error was 0.91, 1.14, 1.08, 0.89, 1.06, and 0.84 across the six cells, a mean of 0.99 with three of six below one.
...
The correlated bank assigns very low probability to a small number of observed pairs, and this does not affect inference.
Under the joint form the maximized log-likelihood was lower for the correlated bank than for the shuffled bank by 697.1, 701.4, 701.7, and 699.4 at nested samples of 10, 20, 30, and 50 participants — a fivefold increase in sample size moved it by 4.6 log units, which is to say it did not move.
...
What it says is that a handful of observed isotype pairs are close to impossible under the pairing that the companion analysis estimates.
...
It restores coverage to 98.0% and costs a 25% wider interval — a cost that is remarkably stable, varying by 0.8% of itself across an eightfold range of transmission intensity.
...
Both forms estimate the incidence rate to within about two percent, and they differ from each other by less than one percentage point; what differs is the curvature of the log-likelihood, which the product form overstates because it counts one specimen as two independent observations.
...
Applied here it returned a mean ratio of 0.99, and fell below one in half the cells — it detected nothing.
...
The estimand is a single scalar, so the robust variance rests on the between-participant scatter of one score contribution, and with a few hundred participants that quantity is itself noisily estimated [@rotnitzky1990].
...
Replacing the correlated bank with one that has the same margins and no pairing was predicted to leave the product form unchanged, and it changed it by exactly zero in six cells of six — a departure would have meant the pairing was entering somewhere it could not legitimately enter and would have invalidated every contrast in the chapter (@sec-ch3-res-valid).
...
That is not what the results show, and the distinction is worth stating plainly because it changes what the method is for.
...
@eq-two-assumptions separated two changes that the joint form makes together: sharing the infection time, which follows from a participant having had one infection, and pairing the kinetic parameters, which is where the companion analysis's estimate enters.
...
A reader who is unpersuaded by the companion analysis's correlation estimate still has to accept that a participant has one infection time, and three quarters of the effect reported here follows from that alone.
The dependence between the two chapters is real but partial, and it is quantified, not asserted.
...
That analysis is specified in the companion analysis of antibody kinetics [@lee2026correlated].

Agreement between the two would be a second measurement of the same quantity by a route that shares neither the data nor the estimand, and would put the attribution above on firmer ground than one contrast can.
...
The joint estimate exceeded the product estimate in every cell, by 29% on average, and the simulation does not reproduce this.
...
**A parameter bank from confirmed cases.** The kinetics are fitted to blood-culture-confirmed infections, which are more severe than the subclinical and mild infections that dominate a population serosurvey.
If community responses are smaller and more variable, the bank will overstate how tightly the isotypes travel together.

Until this is settled, no claim about the level of incidence should rest on that shift.
...
With a single biomarker nothing changes; the forms are identical (@eq-threeform).
The question arises only when two or more are combined.
...
The joint form is slightly conservative, covering above nominal, and whether the excess is a property of the estimator or an artifact of the finite parameter bank is not settled; increasing the bank size and re-examining the coverage is the direct test.
The companion analysis models correlation at one level of the hierarchy and not two, leaving residual measurement noise uncorrelated, so the fitted parameter correlation absorbs any concordance arising at the measurement level and simulating from it then over-predicts concordance in a study whose measurement-level component differs.
The point-estimate shift is not explained: the joint form gives estimates about a third higher on the observed data and the simulation does not reproduce this (@sec-ch3-res-shift), so it is reported as an open finding and no claim about the level of incidence rests on it.
...
Age stratification is the most direct way to separate the two explanations above: if repeated infection is the cause, the two isotypes should come apart more in older participants, and because seroincidence can be estimated within age strata from an unstratified parameter bank this requires no refitting, only additional survey cells.
...
More than two biomarkers, where the independence assumption is applied more times and its cost should grow.
Other pathogens and cohorts, since nothing in the derivation is specific to enteric fever.
And the discordance of @sec-ch3-discussion, which is the most interesting thing this chapter found and the thing it is least able to explain: if repeated infection is the cause, then the single-infection framework itself — shared by every likelihood form considered here — is the next assumption to examine.
...
The first would be a negative result about the companion analysis's applicability; the second is a statement about a handful of participants and belongs in the limitations.
