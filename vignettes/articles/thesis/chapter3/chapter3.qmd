...
Cross-sectional serosurveys can estimate the seroincidence rate of enteric fever by leveraging antibody kinetics estimated from confirmed cases.
...
We applied all three forms to the Seroepidemiology and Environmental Surveillance for Enteric Fever (SEES) cross-sectional serosurveys, covering 2,613 participants in randomly selected population samples in Bangladesh, Nepal, and Pakistan.

An interval constructed to have 95% coverage contained the true seroincidence 89.7% of the time when built by adding log-likelihoods; the joint form improved coverage to 98.0%, at a cost in width of about a quarter that held across the range of seroincidence examined.
...
Sharing the infection time accounted for 76% of the correction; pairing the parameter draws accounted for the remaining 24%.
Applied to the serosurveys, every interval widened.
The ranking of countries and the high- versus low-endemicity contrasts did not change.

Seroincidence remains a useful, lower-cost approach for ranking enteric fever burden and supporting vaccine-prioritization decisions.
However, when multiple biomarkers are combined, accounting for their shared infection history widens confidence intervals and suggests that the current method likely reports uncertainty that is too narrow.
...
The kinetics analysis that supplies the parameter draws fitted its models in Stan [@stan2024; @lee2026correlated]; incidence estimation uses the product and joint likelihood implementations developed for this analysis; `serocalculator` [@serocalculator2025] is used for the single-biomarker equivalence check and for the reference comparison of @tbl-ref.
...
We will deposit the analysis code in a public repository at the time of publication.
