...
ones [@serocalculator2025]. The package's issue tracker records the discrepancy as issue 637, and an implementation of the shared-time form is open there as issue 646.
What
...
@fig-ch3-flow gives the number remaining after each step.
...
The three are referred to throughout as forms
A, B, and C, with
...
out <- data.frame(
  ...
  check.names = FALSE)
thesis_ft(out, widths = c(1.05, 0.80, 0.50, 0.95, 0.95, 0.80, 1.00))
```
...
## Implementation verification {#sec-ch3-res-valid}
...
## Application to serosurvey data {#sec-ch3-res-app}

@tbl-forms gives the three forms in all six sites and @fig-forest displays them.
...
The gap from A to B is what sharing the infection time is
...
That is the whole reason the shared-time form is computed: the shared-time form isolates the first assumption, so its difference from the product form is what the shared infection time is worth and $\mathrm C-\mathrm B$ is what the pairing adds on top of it.
...
An earlier version of this contrast, computed with the established implementation in the role of the product form, gave 79% for the sharing axis. **The internal comparison gives 76%.** Two routes that share no code and no data agree to three percentage points.
...
The joint estimate exceeded the product estimate in six of six sites, by 14.2% to 42.4%, with a mean of 28.8%.
...
## Sensitivity analyses {#sec-ch3-res-sens}
...
It restores coverage to 98.0% and costs a 25% wider interval — a cost that varies by 0.8% of itself across an eightfold range of seroincidence.
...
The joint estimate exceeded the product estimate in every site, by 29% on average, and the simulation does not reproduce this.
...
The point-estimate shift is not explained: the joint form gives estimates about a third higher on the observed data and the simulation does not reproduce this (@sec-ch3-res-shift), so we report it as an open finding and no claim about the level of incidence rests on it.
...
More than two biomarkers, where the estimator applies the independence assumption more times and its cost should grow.
...
- offset grows in proportion to $n$ — the deficit is spread across participants, and the paired draws genuinely explain the data less well;
