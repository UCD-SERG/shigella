...
We applied all three forms to the Seroepidemiology and Environmental Surveillance for Enteric Fever (SEES) cross-sectional serosurveys, covering 2,613 participants in Bangladesh, Nepal, and Pakistan.
...
and against the existing compiled implementation rather than against a second implementation written for this analysis.
...
Kinetic models use the `serodynamics` package; incidence estimation uses the product and joint likelihood implementations developed for this analysis; `serocalculator` [@serocalculator2025] is used for the single-biomarker equivalence check and for the reference comparison of @tbl-ref.
