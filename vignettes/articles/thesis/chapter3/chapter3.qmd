...
A statement that one interval is half again as wide as another is a statement about that code.
...
## Ethical approval {#sec-ch3-ethics}
...
That is the whole reason the shared-time form is computed: the shared-time form isolates the first assumption, so its difference from the product form is what the shared infection time is worth and its difference from the joint form is what the pairing adds on top of it.
...
The joint estimate exceeded the product estimate in six of six sites, by 14.2% to 42.4%, with a mean of 28.8% relative to the shared-time form.
...
The joint estimate exceeded the product estimate in every site, by 29% on average relative to the shared-time form, and the simulation does not reproduce this.
...
And the discordance discussed below, which is the most interesting thing this chapter found and the thing it is least able to explain: if repeated infection is the cause, then the single-infection framework itself — shared by every likelihood form considered here — is the next assumption to examine.

## Funding {#sec-ch3-funding}
