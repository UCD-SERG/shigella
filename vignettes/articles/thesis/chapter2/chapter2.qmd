...
We ran four fits and chose the primary before comparing the estimates.
We ran each fit for a stated reason and in sequence, not until one gave a preferred answer.
...
Given a posterior standard deviation of 0.054 and bulk effective sample sizes of 45 and 76 in those fits, that difference is seven standard errors and so is not Monte
Carlo noise.
...
The only
prior this chapter introduces is the one on $\rho_c$, and we apply it identically in both fits.
...
The quantity to read is the range across fits for each parameter, not any single fit.
...
Retained-chain counts differ across fits because the same rule that removes minor-mode chains also removes chains that fail on their own terms.
