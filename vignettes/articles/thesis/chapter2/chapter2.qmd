...
We fitted the model to 461 blood-culture-confirmed enteric fever cases from Bangladesh, Nepal, and Pakistan, with HlyE IgG and IgA measured at a median of four visits each over two years.

All five correlations excluded zero, with posterior medians from 0.64 to 0.84 and a smallest lower bound of 0.53.
The same association is present in the measurements themselves with no model involved, at rank correlations of 0.51 to 0.67 across four features that can be read directly from a participant's own series.
...
@fig-ch2-flow gives the number remaining after each step.
...
**It is not tuning.** The threshold is not an influential quantity: we report results at 50, 100 and 200 and they agree to three decimal places, so no choice within that range changes a conclusion.
It is
reported as a sensitivity rather than asserted as a constant fixed beforehand.

**It is not convergence repair.** Within each mode the sampler is well behaved.
The rule selects
...
We ran four arms and chose the primary before comparing the estimates.
We ran each arm for a stated reason and in sequence, not until one gave a preferred answer.
...
**The null condition is not the independent model refitted.** Refitting would produce its own population means, within-isotype blocks and measurement precisions, so a comparison against it would confound the cross-covariances with those differences.
...
falls.
We recorded the rule before running the grids, and we report it as failing where it fails
...
@tbl-ch2-sens lists the analyses, what each varies, and what each would have shown had it failed.
...
That separation licenses the choice of primary arm.
...
typical set.
The arm at a starting scale of 3 distinguishes them.
...
The prediction recorded before these arms were run
...
error can account for, and @sec-ch2-res-weak takes that parameter up separately.

## Recovery of the correlations under simulation {#sec-ch2-res-sim}

@tbl-ch2-cov gives coverage, bias, and interval width under both conditions and all three chain-handling schemes.
...
@tbl-ch2-cov pools the five parameters into one figure per grid and
...
The pre-specified decision rule was not met in any of the six parameter-by-condition combinations (@sec-ch2-sim), and every
...
The two grids move in different directions.
...
Decay-shape covers 1.000 with the second-smallest mean squared error.
...
Given a posterior standard deviation of 0.054 and bulk effective
...
@fig-ch2-pairs is not independent evidence that the correlations are real, because the figure is persuasive in a way the numbers behind it do not support.
...
\qquad
\underbrace{\text{with correlation: multimodality}}_{\text{the filter removes it}}
$$
...
It has the lowest coverage of the five when correlation is present (@tbl-ch2-perparam); it shows the largest disagreement across arms, and by a margin too wide for Monte Carlo error to explain (@sec-ch2-res-init); and it is the parameter whose correlation is largest in the observed data (@tbl-rho).
...
@tbl-priors gives every prior in the model, with the value used and the reason for it.
...
The only
prior this chapter introduces is the one on $\rho_c$, and we apply it identically in both arms.
...
@tbl-init gives the baseline correlation across the four starting scales; @tbl-init-full gives the full set.
...
Across the
four initialization arms, which share a prior, the medians move by 0.003 to 0.016.
Across the three
...
Retained-chain counts differ across arms because the same rule that removes minor-mode chains also removes chains that fail on their own terms.
...
the product likelihood and the joint likelihood implementation developed for the seroincidence analysis.
