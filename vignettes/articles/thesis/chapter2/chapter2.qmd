...
## Ethical approval {#sec-ch2-ethics}
...
# The run identifiers are dropped: the starting scale is the thing that varies, and
# it is already a column of its own.
d <- as.data.frame(do.call(rbind, list(
...
thesis_ft(d, widths = c(1.25, 1.15, 1.50, 0.65, 0.70, 0.85))
```
...
the tight one, against 0.831 in the primary arm, and across all five parameters the movement is 0.020 to 0.090 (@sec-appA-init).
...
#|   filtering. The first four columns give the starting scale and share the prior and the chain
#|   count, and the range is over those four; the last two widen and tighten the prior
