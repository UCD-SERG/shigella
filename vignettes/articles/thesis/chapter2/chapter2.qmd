...
Seroincidence estimation reads the rate of new infections in a population from antibody concentrations measured once per participant, and it rests on a model of how those concentrations change after infection.
That model is fitted to each antigen–isotype combination separately, so a participant's IgG and IgA are treated as though they came from unrelated people.
They do not: they are measured on one specimen, from one person, following one infection.

We fitted a hierarchical two-phase kinetic model to 461 blood-culture-confirmed enteric fever cases from Bangladesh, Nepal, and Pakistan, with HlyE IgG and IgA measured at a median of four visits each over two years.
To the existing covariance structure we added five cross-isotype correlations, one for each kinetic parameter, leaving everything else unchanged, so that setting them to zero recovers the previous model exactly and any difference in fit is attributable to them alone.

All five correlations excluded zero, with posterior medians from 0.64 to 0.84 and a smallest lower bound of 0.53.
The same association is present in the measurements themselves with no model involved, at Spearman coefficients of 0.51 to 0.67 across four features that can be read directly off a participant's series.
...
A simulation study at the observed sample size confirmed that the estimator recovers a correlation that is present and does not manufacture one that is absent.
Out-of-sample comparison favored the correlated model at both aggregations examined.

A participant's antibody isotypes are linked at the level of the individual response, and the linkage is large enough to change what the model says.
Any procedure that combines the two isotypes as independent evidence overstates the precision of what it returns.
...
Parameters are therefore estimated less precisely than the data allow — and, as the companion analysis shows, the incidence estimates that follow are more confident than the data justify.
...
Arm T and arm S are also the two objects the companion analysis of seroincidence draws its correlated and independent parameter banks from [@lee2026seroincidence].
...
What it does show is the pairing the fitted model carries, which is the object the companion analysis draws from when it pairs parameter draws within a simulated participant.
...
The cluster-robust standard error in the companion analysis was predicted to exceed one and did not [@lee2026seroincidence]; the participant-level comparison here was expected to be the primary evidence for the extension.
...
The distinction is the one the companion analysis draws for the incidence interval [@lee2026seroincidence], reached here from the opposite direction — there an interval was too narrow and correctly centered, here it is wide enough and off-center.
...
Neither enters the companion analysis.
...
That is not a defect in either chapter; it is a fact about the estimator, and it predicts what the companion analysis finds.
...
The companion analysis asks it at the population level and finds that the pairing accounts for roughly a quarter of the correction it reports, with the larger part following from a participant having a single infection time [@lee2026seroincidence].
...
That information can be held out and each model asked to recover it from a single specimen, in three variants that separate the same two assumptions the companion analysis separates: one biomarker alone; two biomarkers under a single shared infection time with independent kinetic parameters; and two with the parameters paired.

The prediction, recorded before the analysis is run, is that the ordering will match that of the companion analysis — the larger improvement from using two biomarkers under one infection time, a smaller further improvement from pairing them.
...
If instead the pairing supplies most of the individual-level gain, the two measurements disagree, and the attribution in the companion analysis would have to be revisited before either is relied on.
...
Writing $\boldsymbol\Sigma_{\text{ch2}}$ down as in @eq-blocks and factoring it would require a constraint on $\mathbf c$: by the Schur complement, the $10\times10$ matrix is positive definite if and only if
...
Two consequences are worth stating because they are easy to misread.
...
The estimand is therefore computed from the assembled $\boldsymbol\Sigma$ and is bounded by $\pm1$ automatically, whatever the sampled $\rho_{c,j}$ does.
