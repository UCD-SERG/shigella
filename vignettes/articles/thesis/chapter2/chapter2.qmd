---
title: "A cross-isotype correlation model for post-infection antibody kinetics"
author: "Kwan Ho Lee"
date: today
lang: en
...
Enteric fever is a systemic bacterial infection caused by *Salmonella enterica* serovars Typhi and Paratyphi.
Understanding post-infection antibody responses is valuable for surveillance and diagnostics because antibody concentrations preserve information about recent infection after the infection itself is no longer observed.
A two-phase within-host model has been developed to characterize humoral antibody responses after enteric fever infection using five kinetic parameters: the pre-infection concentration, peak concentration, time to peak, decay rate, and decay shape.
Existing applications of this model have treated antibody isotypes as independent.
We relaxed that assumption by adding five cross-isotype correlations, one for each kinetic parameter, to the between-participant covariance structure, leaving every other component unchanged; setting the five to zero recovers the previous model exactly, so any difference in fit is attributable to them alone.
...
All five model-estimated cross-isotype correlations were positive and excluded zero, with posterior medians from 0.64 to 0.84.
The same qualitative pattern was visible in the observed data without using the model: participant-level summaries read directly from the IgG and IgA observations—minimum concentration, maximum concentration, timing of the maximum, and post-peak slope—had Spearman correlations of 0.51 to 0.67.
...
Separately fitted kinetic models can describe the marginal IgG and IgA trajectories, but they miss a central feature of the response: within participants, the two isotypes are correlated across all five kinetic parameters.
Treating those paired responses as independent therefore overstates the amount of information in the data and can make downstream estimates appear more precise than they are.
