...
Each sensitivity analysis varies one component of the specification and would have shown one thing had it failed (@tbl-ch2-sens).
...
The five parameters are pooled into one figure per condition and scheme in
@tbl-ch2-cov; this panel keeps them apart, which is where the two extremes of
@tbl-ch2-perparam become visible.
:::
...
Every prior in the model is set out in @tbl-priors, with the value used and the reason for it.
...
The baseline correlation across the four starting scales appears in @tbl-init, and the full set of five in @tbl-init-full.
