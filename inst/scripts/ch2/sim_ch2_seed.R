...
suppressPackageStartupMessages({library(posterior); library(dplyr)})
...
say <- function(...) { cat(sprintf("[sim %03d] ", SEED), ..., "\n", sep = ""); flush.console() }
...
P <- 5L; K <- 2L                      # 5 kinetic parameters, 2 biomarkers
say("seed ", SEED, " | rho_true ", RHO_TRUE, " | truth_c ", TRUTH_C,
...
if (identical(TRUTH_CH, "lp")) {
  ...
  for (i in o) say(sprintf("    chain %2d   lp %9.0f   %+.0f from best   baseline %+.3f",
...
if (PREC_MULT != 1)
  say("prec_logy scaled by ", PREC_MULT, " -> ", paste(sprintf("%.2f", prec), collapse = " "),
...
fix_corr <- function(L, nm) {
  R <- L %*% t(L)
  d <- sqrt(diag(R)); R <- R / outer(d, d); diag(R) <- 1
...
Lg <- fix_corr(Lg_raw, "Lcorr_G"); La <- fix_corr(La_raw, "Lcorr_A")
...
for (kk in 1:K) {
  ...
  ly1 <- log(exp(p[1]) + exp(p[2])); a <- exp(p[5])
  say(sprintf("truth %s: y0 %.1f | y1 %.1f | t1 %.1f d | shape %.2f | k %.4f/d",
...
mu_flat <- c(mu_G, mu_A)
truth <- list(seed = SEED, rho_true = RHO_TRUE, truth_c = TRUTH_C,
...
real  <- readRDS(DATA)
cols  <- list(id = "id", t = "dayssincefeveronset", y = "result",
              bio = "antigen_iso")
sched <- real |>
...
N <- length(sched)
say("design: ", N, " subjects, visits ", min(lengths(sched)), "-", max(lengths(sched)),
...
curve_lp <- function(t, p) {
  ly0 <- p[1]; ly1 <- log(exp(p[1]) + exp(p[2])); t1 <- exp(p[3]); a <- exp(p[5])
  la  <- p[4] - a * ly1
  ifelse(t <= t1,
         ly0 + (ly1 - ly0) / t1 * t,
...
for (i in seq_len(N)) {
  ...
  for (k in seq_len(K)) {
    p  <- PAR[i, ((k - 1) * P + 1):(k * P)]
    mu <- curve_lp(tt, p)
    out[[k]] <- data.frame(id = i, dayssincefeveronset = tt,
                           antigen_iso = iso[k],
...
for (b in iso) {
  ...
  say(sprintf("  log10 %s  simulated %.2f / %.2f / %.2f   real %.2f / %.2f / %.2f   median gap %+.2f",
...
if (!gate_ok)
  stop("simulated levels do not match the real data (median off by more than ",
       ...
       "and compare them with fit$summary(\"mu_par\").", call. = FALSE)
...
if (DRY_RUN) {
  ...
  if (!has_w && WISHDF != 20)
    stop("WISHDF=", WISHDF, " requested but run_mod_stan_ch2() cannot receive it.",
         call. = FALSE)
  say(""); say("DRY RUN: truth extracted, data simulated, level gate passed.")
  say("Re-run without DRY_RUN to fit."); quit(save = "no", status = 0)
}
...
if ("wishdf_param" %in% names(formals(run_mod_stan_ch2))) {
  ...
  stop("run_mod_stan_ch2() has no wishdf_param argument, so WISHDF=", WISHDF,
       ...
       "  prep_ch2_priors(), or drop WISHDF from this call.", call. = FALSE)
} else {
...
scored <- try({
  ...
  ss <- f$summary(vars, mean = mean, median = stats::median, sd = stats::sd,
...
ss <- scored$ss; bych <- scored$bych
say(""); say("=== seed ", SEED, " ===")
say(sprintf("  divergent %d | treedepth %.1f%% | E-BFMI<0.3 %d/%d | max R-hat %.3f | min ESS %.0f",
...
say(""); say("  cross_corr recovery")
cat(sprintf("    %-14s %7s %8s %8s %9s %8s\n",
            "parameter", "truth", "mean", "bias", "90% cov", "R-hat"))
for (i in seq_len(nrow(cc)))
  cat(sprintf("    %-14s %7.3f %8.3f %+8.3f %9s %8.3f\n",
              lab[i], cc$truth[i], cc$mean[i], cc$bias[i],
              ifelse(cc$covered[i], "yes", "NO"), cc$rhat[i]))
say(""); say("done -> ", file.path(OUTDIR, sprintf("res_seed%03d.rds", SEED)))
