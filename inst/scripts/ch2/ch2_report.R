...
SIM_DIRS <- strsplit(Sys.getenv(
  "SIM_DIRS",
  "sim_out_null_prod,sim_out_alt_prod"
), ",")[[1]]
REAL_FITS <- strsplit(Sys.getenv(
  "REAL_FITS",
  "fit_ch2_461_T.rds,fit_ch2_461_T2.rds,fit_ch2_461_T3.rds"
), ",")[[1]]
...
summ <- function(x) {
  c(
    med = median(x), q05 = unname(quantile(x, .05)),
...
for (d in SIM_DIRS) {
  ...
  if (!length(res)) {
    say("\n", d, ": no res_seed to read truth from")
    next
  }
  r1 <- readRDS(res[1])
    ...
    if (!QUIET) {
      cv <- sapply(c("all", "lp", "stacking"), function(sc) {
        sum(sapply(1:5, function(j) {
          q <- summ(dw[[j]][[sc]])
          truth[j] >= q["q05"] && truth[j] <= q["q95"]
        }))
      })
      cat(sprintf(
        "    seed %03d   keep %2d/%2d   w_eff %4.1f   포함 %d / %d / %d\n",
...
for (d in names(grids)) {
  g <- grids[[d]]
  ni <- length(unique(g$seed)) * 5
  for (sc in c("all", "lp", "stacking")) {
    p <- mean(g$covered[g$scheme == sc])
    se <- sqrt(p * (1 - p) / ni)
    cat(sprintf(
      "    %-22s %-9s %.3f   |diff| %.3f   2MCSE %.3f   %s\n",
...
for (d in SIM_DIRS) {
  ...
  if (!length(res)) next
  truth <- readRDS(res[1])
  truth <- truth$truth[grepl("cross_corr", truth$variable)]
  cov_at <- function(L) {
    h <- 0
    n <- 0
    for (p in fits) {
      ...
      for (j in 1:5) {
        ...
        h <- h + (truth[j] >= q["q05"] && truth[j] <= q["q95"])
        n <- n + 1
      }
    }
    if (n == 0) {
  ...
  cat(sprintf(
    "  %-22s %10.3f %10.3f %10.3f %10.3f   (%d of %d scored)\n", d,
...
if (REAL) {
  ...
  for (p in REAL_FITS) {
    ...
    if (length(minor)) {
      cat(sprintf(
        "      minor-mode chains: %s   lp gap %s   stacking weight %.4f\n",
    ...
    cat(sprintf(
      "      largest median difference across schemes: %.3f\n",
...
saveRDS(list(
  grids = grids, summary = do.call(rbind, final),
  lp_drop = LP_DROP, when = Sys.time()
), "ch2_report.rds")
