...
tab <- data.frame(chain = seq_len(K),
...
if (length(disc))
  for (i in disc)
    cat(sprintf("    chain %2d  lp %+6.0f  base %+.3f  lp_keep %s  stack_w %.5f\n",
...
cat(sprintf("\n  %-12s %26.3f %26.3f %26.3f\n", "mean width",
            mean(sapply(1:5, function(j) {x <- cc[, j]; diff(quantile(x, c(.05,.95)))})),
            mean(sapply(1:5, function(j) {m <- matrix(cc[,j], nrow=nit, ncol=K)
...
saveRDS(list(path = path, lp = lp, baseline = base_k, keep = keep,
             w = w, pareto_bad = kbad, lp_drop = LP_DROP),
        sub("\\.rds$", "_stack.rds", basename(path)))
cat(sprintf("\nsaved -> %s\n", sub("\\.rds$", "_stack.rds", basename(path))))

cat("\nPareto k: ", sprintf("%.0f%% of observations exceed 0.7 on average across chains\n",
                            100 * mean(kbad)))
