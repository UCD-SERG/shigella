...
extract_par_new <- function(fit) {
  ...
  if (all(grepl("^par_new\\[[0-9]+\\]$", cn))) {
    idx <- list(HlyE_IgG = sprintf("par_new[%d]", 1:5),
                HlyE_IgA = sprintf("par_new[%d]", 6:10))
    layout <- "vector[10]"
  } else if (any(grepl("^par_new\\[[0-9]+,[0-9]+\\]$", cn))) {
    idx <- list(HlyE_IgG = sprintf("par_new[1,%d]", 1:5),
                HlyE_IgA = sprintf("par_new[2,%d]", 1:5))
    layout <- "matrix[2,5]"
  } else {
    say("unrecognised par_new naming:"); print(head(cn, 12)); stop(call. = FALSE)
  }
  miss <- setdiff(unlist(idx), cn)
  if (length(miss)) { say("missing columns: ", paste(miss, collapse = " ")); stop(call. = FALSE) }
...
to_curve_params <- function(fit, n_draws, lp_drop) {
  ...
  say(sprintf("  chains %d kept of %d   (lp spread %.0f, threshold %g)",
              length(keep), K, max(lp) - min(lp), lp_drop))
  if (length(keep) < K) {
    d <- setdiff(seq_len(K), keep)
    say(sprintf("    dropped: %s",
  ...
  out <- do.call(rbind, lapply(names(P$idx), function(nm) {
    ...
    storage.mode(p) <- "double"; dimnames(p) <- NULL
    y0 <- exp(p[, 1]); y1 <- y0 + exp(p[, 2]); t1 <- exp(p[, 3])
    kd <- exp(p[, 4]); r  <- exp(p[, 5]) + 1
    data.frame(antigen_iso = nm, iter = seq_along(rows),
               y0 = y0, y1 = y1, t1 = t1,
               alpha = kd / y1^(r - 1), r = r,
               row.names = NULL)
  }))
...
check_layout <- function(cp) {
  ...
  for (nm in names(EXPECT)) {
    ...
    got <- c(y0 = median(d$y0), y1 = median(d$y1), t1 = median(d$t1),
             r = median(d$r), kdec = median(d$alpha * d$y1^(d$r - 1)))
    exp_ <- EXPECT[[nm]]
    ratio <- got / exp_
    say(sprintf("  %-9s y0 %6.1f  y1 %7.1f  t1 %5.1f d  r %5.2f  k %7.4f/d",
                nm, got["y0"], got["y1"], got["t1"], got["r"], got["kdec"]))
    say(sprintf("  %-9s expected %5.1f   %7.1f    %5.1f      %5.2f    %7.4f",
                "", exp_["y0"], exp_["y1"], exp_["t1"], exp_["r"], exp_["kdec"]))
    bad <- names(ratio)[ratio < 0.4 | ratio > 2.5]
    if (length(bad)) {
      ok <- FALSE
      say(sprintf("  *** %s off by more than a factor of 2.5: %s ***",
...
report_correlation <- function(cp, label) {
  ...
  say(sprintf("    y0 %+.3f   y1 %+.3f   t1 %+.3f   alpha %+.3f   r %+.3f",
...
for (arm in names(ARMS)) {
  ...
  hr(); say(sprintf("arm %s  (%s)   %s", arm, ARMS[[arm]], p))
  if (!file.exists(p)) { say("  not found; skipping"); next }
  ...
  attr(cp, "source") <- sprintf("%s, seed %s, %d chains, %d kept",
                                p, unique(fit$metadata()$seed),
                                length(fit$metadata()$id),
                                length(attr(cp, "chains_kept")))
  saveRDS(cp, fn)
  say(sprintf("  -> %s   %d draws x 2 isotypes   %.2f MB",
...
if (length(res) == 2) {
  ...
  for (a in names(res))
    say(sprintf("  %-8s %+8.3f %+8.3f %+8.3f %+8.3f %+8.3f",
  ...
  if (max(abs(s_main)) > 0.10)
    ...
    say(sprintf("  arm S is near zero on the four sampled parameters (max %.3f);\n  alpha is %+.3f. Run R/06_check_bank_correlation.R before using this.",
