...
KEEP <- c("pid", "index_id", "Country", "sees_site", "studyarm", "studyvisit",
          "antiso", "antigen", "isotype", "Age", "sampleage",
          "result", "result.llod", "llod", "sex", "catchment")

say("reading ", ELISA)
raw <- readr::read_csv(path.expand(ELISA), show_col_types = FALSE,
                       col_select = any_of(KEEP), progress = FALSE)
say("  ", format(nrow(raw), big.mark = ","), " rows, ", ncol(raw), " columns")

cascade <- tibble::tibble(step = character(), rows = integer(),
...
d <- raw |>
  ...
  add("population survey arms (highE, lowE)",
...
both <- d |> count(pid, name = "n_iso") |> filter(n_iso == 2)
d <- d |>
...
# ---- report -----------------------------------------------------------------
hr(); say("FILTER CASCADE"); hr()
print(as.data.frame(cascade), row.names = FALSE)

hr(); say("ANALYSIS SET"); hr()
say("  people ", format(n_distinct(xs$id), big.mark = ","),
...
print(xs |> group_by(antigen_iso, arm) |>
        summarise(n = n(), at_llod = sum(value <= llod * 1.001),
...
hr(); say("LIMITS OF DETECTION BY COUNTRY")
...
if (requireNamespace("serocalculator", quietly = TRUE)) {
  ...
  chk <- xs |> distinct(Country, antigen_iso, llod) |>
