...
suppressPackageStartupMessages({
  library(dplyr); library(serocalculator)
})
...
banks <- list(S     = chr(readRDS(file.path(DATA, "curve_params_arm_S.rds"))),
...
if (!all(BANKS %in% names(banks)))
  stop("unknown bank in BANKS: ",
       paste(setdiff(BANKS, names(banks)), collapse = ", "), call. = FALSE)
banks <- banks[BANKS]
...
for (b in names(banks)) {
  cp <- banks[[b]]
  g <- cp[cp$antigen_iso == "HlyE_IgG", ]; g <- g[order(g$iter), ]
  a <- cp[cp$antigen_iso == "HlyE_IgA", ]; a <- a[order(a$iter), ]
  say(sprintf("  arm %s  %d draws   cor(log y1) %+.3f   cor(log alpha) %+.3f",
...
for (arm in ARMS) for (ctry in ctry_list(arm)) {
  ...
  if (nrow(np) != 2) { say("  skip ", arm, "/", ctry, ": noise rows ", nrow(np)); next }

  pd <- as_pop_data(d, age = "age", value = "value", id = "id",
                    antigen_isos = ISOS)

  hr("="); say(sprintf("%s / %s   %d people", arm, ctry, n_distinct(d$id))); hr("=")
  ...
  for (b in names(banks)) {
    ...
    if ("composite" %in% FORMS) {
      t0 <- Sys.time()
      A  <- profile_ci(function(l, ...) serocalculator::log_likelihood(
              l, pd, cp, np, antigen_isos = ISOS), LAMS)
      say(sprintf("  bank %s   composite  %.4f [%.4f, %.4f]  width %.4f   (%.1f min)",
    ...
    if ("joint" %in% FORMS) {
      t0 <- Sys.time()
      C  <- profile_ci(function(l, ...) log_likelihood_joint(
              l, d, banks[[b]], np, antigen_isos = ISOS, n_tau = N_TAU), LAMS)
      say(sprintf("           joint      %.4f [%.4f, %.4f]  width %.4f   (%.1f min)",
  ...
  ok <- tryCatch({
    ...
    e1 <- est_seroincidence(pop_data = pd, sr_params = as_sr_params(banks$T),
                            noise_params = np, antigen_isos = ISOS)
    e2 <- est_seroincidence(pop_data = pd, sr_params = as_sr_params(banks$T),
                            noise_params = np, antigen_isos = ISOS,
                            cluster_var = "id")
    s1 <- summary(e1); s2 <- summary(e2)
    ...
    res[[length(res) + 1]] <- data.frame(
      ...
      row.names = NULL)
    TRUE
  }, error = function(e) { say("\n  sandwich unavailable: ", conditionMessage(e)); FALSE })
}
...
hr("="); say("THE FOUR CONTRASTS"); hr("=")
w <- function(a, c_, f) {
  x <- out |> filter(arm == a, Country == c_, bank == "S" | bank == "T")
  x
}
for (a in unique(out$arm)) for (ct in unique(out$Country[out$arm == a])) {
  x <- out |> filter(arm == a, Country == ct)
  g <- function(bk, fm) { v <- x$width[x$bank == bk & x$form == fm]; if (length(v)) v[1] else NA }
  pc <- function(p, q) if (is.na(p) || is.na(q)) NA else round(100 * (p - q) / q, 1)
  say(sprintf("  %s / %s", a, ct))
  say(sprintf("    sharing t      (joint,S) vs (comp,S)   width %+.1f%%",
              pc(g("S","joint"), g("S","composite"))))
  say(sprintf("    corr + margins (joint,T) vs (joint,S)     width %+.1f%%",
              pc(g("T","joint"), g("S","joint"))))
  say(sprintf("    correlation    (joint,T) vs (joint,Tshuf) width %+.1f%%",
              pc(g("T","joint"), g("Tshuf","joint"))))
  say(sprintf("    PREDICTED = 0  (comp,T)  vs (comp,Tshuf)  width %+.1f%%",
  ...
  if (length(sr) && is.finite(sr)) {
    say(sprintf("    sandwich       SE ratio %.2f   (%.5f -> %.5f)",
                sr / sn, sn, sr))
