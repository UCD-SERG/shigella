...
G <- cp[cp$antigen_iso == ISOS[1], ]
G <- G[order(G$iter), ]
A <- cp[cp$antigen_iso == ISOS[2], ]
A <- A[order(A$iter), ]
...
hr("=")
say("COVERAGE STUDY  --  stage: ", STAGE)
hr("=")
say(sprintf(
  "  bank %s   %d paired draws   columns: %s",
  BANK, NDRAW, paste(names(G), collapse = ", ")
))
say(sprintf(
  "  country %s   y.low %s   nu %s   eps %s", COUNTRY,
  paste(sprintf("%.3f", noise$y.low), collapse = "/"),
  paste(sprintf("%.3g", noise$nu), collapse = "/"),
  paste(sprintf("%.3g", noise$eps), collapse = "/")
))
say(sprintf(
  "  true lambda %.4f   grid %d points over %.4f to %.4f (%.1f%% apart)",
  LAMBDA, N_LAM, min(LAMS), max(LAMS), STEPP
))
say(sprintf(
  "  ages resampled from %d participants, median %.1f",
...
say(sprintf(
  ...
  if (as.integer(Sys.getenv("TAU_INDEP", "0")) == 1L) {
    ...
    "shared within participant  (one infection, one tau)"
  }
))
...
sim_xs <- function(lambda, n, seed) {
  ...
  for (j in seq_along(ISOS)) {
    ...
    y <- pmax(y, nz$y.low)
    out[[j]] <- data.frame(
      id = seq_len(n), age = age, antigen_iso = ISOS[j],
...
.fit_form <- function(d, form) {
  e <- .lik(form)
  profile_ci(function(l) {
    e$log_likelihood_joint(
      l, d, cp, noise,
...
if (STAGE == "discord") {
  hr("=")
  say("DISCORDANCE -- observed versus simulated under arm_T")
  hr("=")
  ...
  disc_stats <- function(d, lo) {
    ...
    hi_a <- a > median(a)
    c(
      cor = r, resid_sd = rs, both_at_limit = mean(cg & ca),
  ...
  for (cell in strsplit(CELLS, ",")[[1]]) {
    ...
    lo <- c(
      noise$y.low[noise$antigen_iso == ISOS[1]],
    ...
    say(sprintf(
      "  %-22s %6d %8s %9.3f %9.3f %8.1f%% %8.1f%%",
      paste(arm, ctry), n_i, "observed",
      o["cor"], o["resid_sd"], 100 * o["both_at_limit"], 100 * o["crossed"]
    ))

    S <- vapply(seq_len(NREP), function(b) {
      disc_stats(sim_xs(lam, n_i, SEED0 + 7000 + b), lo)
    }, numeric(4))
    m <- rowMeans(S, na.rm = TRUE)
    s <- apply(S, 1, sd, na.rm = TRUE) / sqrt(NREP)
    say(sprintf(
      "  %-22s %6s %8s %9.3f %9.3f %8.1f%% %8.1f%%   (MCSE %.3f)",
      "", "", "arm_T sim", m[1], m[2], 100 * m[3], 100 * m[4], s[1]
    ))
    say(sprintf(
      "  %-22s %6s %8s %+9.3f %+9.3f %+8.1f%% %+8.1f%%   <- observed minus simulated",
      "", "", "difference",
...
if (STAGE == "bigN") {
  ...
  hr("=")
  say("LARGE-SAMPLE BIAS -- ", toupper(FORM1), " LIKELIHOOD")
  hr("=")
  say(sprintf(
    "  lambda %.4f   n %d   %d replicates   %s only",
    LAMBDA, NPOP, REPS, FORM1
  ))
  say(sprintf(
    "  grid %d points over %.4f to %.4f (%.2f%% apart)",
  ...
  for (b in seq_len(REPS)) {
    ...
    say(sprintf(
      "  rep %2d  est %.4f  bias %+6.1f%%  width %.4f  %s  %.1f min",
  ...
  hr("=")
  say(sprintf(
    "  n %d   mean estimate %.4f   bias %+.1f%%  (MCSE %.1f%%)  = %.1f SE",
...
")
  saveRDS(
    data.frame(
      lambda = LAMBDA, npop = NPOP, rep = seq_len(REPS),
      est = est, width = wid, covers = cov
    ),
    file.path(OUT, sprintf(
      "bigN_%s_lam%s_n%d.rds",
...
if (STAGE == "selftest") {
  hr("=")
  say("SELF TEST -- form C must recover a lambda drawn from its own model")
  hr("=")
...
")
  ...
  say(sprintf(
    "  %9s %8s %9s %8s %9s %11s %8s", "TAU_SCALE", "lambda",
  ...
  for (sc in scales) {
    ...
    for (lam in lams) {
      ...
      if (!inherits(zp, "try-error")) {
        say(sprintf(
          "        (reference: serocalculator %.4f, ratio %.2f)",
          zp["est"], zp["est"] / lam
        ))
      }
      ...
      say(sprintf(
        "  %9.2f %8.3f %9.4f %8.2f %8.0f%% %10.1f%% %8.1f",
  ...
  hr("=")
  say(sprintf(
    "  best convention: TAU_SCALE = %g   ratios %s",
...
if (STAGE == "jointcheck") {
  hr("=")
  say("JOINT CHECK -- joint likelihood, n = 150, one sample")
  hr("=")
...
hr("=")
say(sprintf(
  "REPLICATES  shard %d of %d  ->  %d reps: %s",
  SHARD, NSHARD, length(myreps),
  paste(range(myreps), collapse = "-")
))
hr("=")
...
OUTF <- file.path(OUT, sprintf(
  "cov_%s%s_lam%s_n%d_sd%d_s%02d.rds",
...
for (rep in myreps) {
  ...
  .all <- list(
    composite = fit_product, sharedtau = fit_sharedtau,
    joint = fit_joint
  )
