...
rd <- function(f) { r <- try(readRDS(f), silent = TRUE)
...
hr("="); say("THREE FORMS, PAIRED ON THE SAME SIMULATED DATASETS"); hr("=")
AC <- load_dir("out/cov"); B <- load_dir("out/covB")
if (is.null(AC)) stop("no files in out/cov", call. = FALSE)
if (is.null(B))  say("  ⚠ out/covB is empty -- form B has not been run")

D <- rbind(AC, B)
D$form <- factor(ifelse(D$form == "composite", "A product",
...
ok <- D %>% group_by(lambda) %>% summarise(k = n_distinct(form), .groups = "drop")
full <- ok$lambda[ok$k == 3]
say("\n  lambdas with all three forms: ",
    if (length(full)) paste(full, collapse = ", ") else "none")

hr("="); say("COVERAGE BY LAMBDA"); hr("=")
S <- D %>% group_by(lambda, form) %>%
  summarise(n = n(), coverage = mean(covers), width = mean(width),
            bias = 100 * (mean(est) / lambda[1] - 1), .groups = "drop") %>%
  mutate(mcse = sqrt(coverage * (1 - coverage) / n))
g <- function(l, f, v) { x <- S[[v]][S$lambda == l & S$form == f]
                         if (length(x)) x[1] else NA_real_ }
lams <- sort(unique(S$lambda))
say(sprintf("  %7s %10s %10s %10s   %10s %10s %10s", "lambda",
            "A", "B", "C", "w A", "w B", "w C"))
for (l in lams)
  say(sprintf("  %7.2f %10.3f %10.3f %10.3f   %10.4f %10.4f %10.4f", l,
...
say(sprintf("  %-14s %7s %10s %8s %9s %10s", "form", "N", "coverage",
...
if ("B shared time" %in% P$form) {
  hr("="); say("HOW THE REPAIR DIVIDES"); hr("=")
  ...
  if (abs(tot) > 1e-9)
    say(sprintf("\n  sharing accounts for %.0f%% of the movement from A to C",
                100 * (b - a) / tot))
  wa <- P$width[P$form == "A product"]; wb <- P$width[P$form == "B shared time"]
...
")
  ...
  hr("="); say("form B not present -- run run_formB_coverage.sh first"); hr("=")
}
hr("=")
